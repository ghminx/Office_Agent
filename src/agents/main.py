...
from src.agents.ecount_calendar import ecount_agent
...
supervisor_builder.add_node("ecount_agent", ecount_agent)  # File search agent node
...
async def run(config):
    # user =  'Ecount에 3일날 등록된 일정 알려줘'
    
    user = "claude code에 대해서 설명해줘"

    # "user" =  "전략기획팀 폴더에서 2026년에 작성한 디딤돌 사업에 있는 파일 어떤거 있는지 확인해줘 pdf만",

    # "user_content": "기존 클라이언트에게 완료된 견적서를 메일로 발송해줘",
    # "user_content": "발주처에게 보고서 결과가 아쉽다는 메일을 보내야하는데 메일 안을 작성해줘",

                    
    # user = """
    #         {
    #                 "user" =  "기존 클라이언트에게 완료된 견적서를 메일로 발송해줘",
